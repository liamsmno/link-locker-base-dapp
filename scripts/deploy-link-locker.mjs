...
const source = await readFile(resolve(process.cwd(), "contracts/LinkLocker.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "LinkLocker.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["LinkLocker.sol"]?.LinkLocker;
if (!contract) {
...
console.log(`Deploying LinkLocker from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_LINK_LOCKER_CONTRACT_ADDRESS to this contract address in Vercel.");
